...
Original file is located at
    https://colab.research.google.com/drive/1QYKCQYDi4NQO_42lF4ov0w8G2KpiXwFV
...
# Usamos 'neg_root_mean_squared_error'. O resultado vem negativo, então multiplicamos por -1.

scores_rmse = cross_val_score(modelo_regressao, df_cleaned[features], df_cleaned[target], cv=5, scoring='neg_root_mean_squared_error')
print("\n--- Validação Cruzada (Regressão) ---")
print(f"RMSE em cada um dos 5 folds: {np.round(-scores_rmse, 2)}")
print(f"RMSE Médio: {(-scores_rmse).mean():.2f}")
print(f"Desvio Padrão dos RMSEs: {(-scores_rmse).std():.2f}")
...
**Resp.:** Mudou bastante! Foi de em torno de 2500 para aproximadamente 3100, um aumento bem significativo. A média de 5 experimentos é mais confiável pois "espalha" os resultados considerados, diminuindo a chance da resposta final ser um outlier.

# Parte 2: Uma Análise Profunda dos Erros de Classificação

A PARTE DOIS FOI FEITA APENAS NO OUTRO NOTEBOOK, COMO SOLICITADO PELO PROFESSOR.

# Relatório de Conclusão

A validação cruzada nos ajudou a avaliar a qualidade e estabilidade do nosso modelo. O RMSE Médio em 5 folds foi de 3086,47; sendo consideravelmente maior do que o RSME com divisão única que foi de 2483,81. Essa diferença reinforça a inportância de utilizar a validação cruzada para evitar que a avaliação seja um outlier.
O Desvio Pedrão dos RMSEs nos 5 folds foi de 870,28. Isso indica que a variabilidade do erro do modelo em diferentes subconjuntos dos dados. Nosso desvio padrão foi alto, quase 30% do RMSE médio, isso sugere que a performance do modelo pode variar dependendo dos dados que ele está usando, o que pode indicar uma alta sensitibilidade a pequenas variações no dataset.
Comprado com a média do preço e do desvio padrão, um RMSE médio de R$3086,47 representa um erro considerável na previsão do preço dos carros, que tem uma médio de R$13.207,13 e um devio padrão de R$7947,07. O modelo tem um erro médio em torno de 0% da médio dos preços, que sugere que ainda há bastante oportunidade de melhoria, além disso, a análise visual do scatter plot da Aula 9 nos indicou, que o modelo tem mais dificuldade para prever carros mais caros, o que contribou para o RMSE elevado.
"""