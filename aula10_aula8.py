...
Original file is located at
    https://colab.research.google.com/drive/1w5HfFQIiNPX5CnMiFPtJIuNRmok-2cTa
...
# Aula 10: Validação e Avaliação Avançada de Modelos
...
# Parte 1: Testando a Robustez do Modelo com Validação Cruzada
"""
...
# 'scoring='accuracy'' é o padrão para classificação.

scores_acuracia = cross_val_score(modelo_arvore, x_encoded, y, cv=5, scoring='accuracy')

print("--- Validação Cruzada (Classificação) ---")
print(f"Acurácias em cada um dos 5 folds: {np.round(scores_acuracia, 3)}")
print(f"Acurácia Média: {scores_acuracia.mean():.2%}")
print(f"Desvio Padrão das Acurácias: {scores_acuracia.std():.4f}")

"""## Pontos de Parada para Discussão em Dupla (Reflexão)

1. A "Acurácia Média" da validação cruzada foi muito diferente da acurácia que encontramos no Aula 8? O que um desvio padrão baixo nos diz sobre a estabilidade do nosso modelo?                
**Resp.:** Foi um pouco mais alta, mas não foi muito diferente, os dois valores são bem próximos. Um desvio padrão baixo nos que nosso modelo é bastante estável, pois todos os resultados são parecidos.

2. Para o modelo de regressão, o "RMSE Médio" mudou muito em relação ao da Aula 9? Por que essa média de 5 experimentos é mais confiável do que o resultado de um único experimento?                  
**Resp.:** Resposta no outro notebook.

# Parte 2: Uma Análise Profunda dos Erros de Classificação
"""
...
1. Olhando a Matriz de Confusão, quantos "Falsos Positivos" (modelo previu Churn, mas cliente não saiu) e quantos "Falsos Negativos" (modelo previu Não Churn, mas cliente saiu) nosso modelo cometeu?           
**Resp.:** 215 falsos positivos e 258 falsos negativos.

2. Para a empresa de telecomunicações, qual erro é mais caro: um Falso Positivo (gastar dinheiro em uma oferta de retenção para um cliente feliz) ou um Falso Negativo (perder um cliente sem fazer nada)?          
**Resp.:** Falso Negativo. Pois a quantidade gasta em oferta deve ser menor do que o lucro gerado por um cliente, por isso, perder um cliente é pior.

3. Com base na sua resposta anterior, qual métrica é mais importante para este problema de negócio: Precisão ou Recall? O que o valor obtido no relatório nos diz sobre o desempenho do nosso modelo nesse quesito?            
**Resp.:** Recall. Pois mostra quantos clientes que sairam o modelo conseguiu idenfiticar corretamente.

# Relatório de Conclusão

Com base na validação cruzada, observamos que a Acurácia Média de 78,13% foi bem próxima à acurácia obtida no treinamento inicial, que foi de 77,61%, além disso, o desvio padrão foi baixo, de 0,0101. Isso tudo indica que nosso modelo é bom e estável, apresentando desepenho consistente em diferentes "rodadas". Isso nos motra que o modelo não sofre de grande overfitting e se comporta bem com novos sets de dados.
Para o problema de Churn, o erro mais problemático é o Falso Negativo, ou seja, quando o modelo diz que um cliente não vai sair (Não Churn) mas na realidade ele sai (Churn Sim). Esse erro é mais caro para a empresa pois significa a perda de um cliente sem que nenhuma medida de retenção tenha sido tentada. Pelo outro lado, quando temos um Falso Positivo, ou seja, prever Churn para um cliente que não vai cancelar, resulta em uma oferta de retenção desnecessária, que ainda perde dinheiro para a empresa, mas menos do que caso o cliente tivesse cancelado.
Analisando o Relatório de Classificação, observamos que a Precisão para a classe do Não Churn é de 0,84 e o Recall para Churn Sim é de 0,54. A Precisão para Não Churn indica que quando o modelo acha que um cliente não vai cancelar, o modelo está correto 84% das vezes. Já o Recall para o Churn Sim india que o modelo identifica corretamente 54% das pessoas que realmente cancelaram. Com base nisso, sabemos que nosso modelo é mais forte em prever a classe marjoritária Não Churn, com uma precisão boa. Porém, sua capacidade de deteccção de clientes que vão cancelar é menor, isso é um ponto a ser melhorado, principlamente tendo em vista que Falsos Negativos são mais graves para a empresa.
"""